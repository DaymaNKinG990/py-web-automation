...
## Overview
Tests for `py_web_automation.clients.db_client.DBClient` class - database client with support for multiple backends (PostgreSQL, SQLite, MySQL).
...
#### TC-DB-MYSQL-ERROR-001: MySQLAdapter _execute_query_pymysql с RuntimeError
- **Purpose**: Verify _execute_query_pymysql raises RuntimeError when connection is None
- **Preconditions**: MySQLAdapter with pymysql, connection is None
- **Test Steps**:
  1. Create MySQLAdapter with pymysql
  2. Set _connection = None
  3. Call await adapter._execute_query_pymysql("SELECT 1", None)
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `_execute_query_pymysql` error handling (line 171)

#### TC-DB-MYSQL-ERROR-002: MySQLAdapter _execute_command_pymysql с RuntimeError
- **Purpose**: Verify _execute_command_pymysql raises RuntimeError when connection is None
- **Preconditions**: MySQLAdapter with pymysql, connection is None
- **Test Steps**:
  1. Create MySQLAdapter with pymysql
  2. Set _connection = None
  3. Call await adapter._execute_command_pymysql("INSERT INTO users VALUES (1)", None)
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `_execute_command_pymysql` error handling (line 214)

#### TC-DB-MYSQL-ERROR-003: MySQLAdapter begin_transaction с RuntimeError для pymysql
- **Purpose**: Verify begin_transaction raises RuntimeError when connection is None (pymysql)
- **Preconditions**: MySQLAdapter with pymysql, connection is None
- **Test Steps**:
  1. Create MySQLAdapter with pymysql
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `begin_transaction` error handling for pymysql (line 230)

#### TC-DB-MYSQL-ERROR-004: MySQLAdapter commit_transaction с RuntimeError для pymysql
- **Purpose**: Verify commit_transaction raises RuntimeError when connection is None (pymysql)
- **Preconditions**: MySQLAdapter with pymysql, connection is None
- **Test Steps**:
  1. Create MySQLAdapter with pymysql
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `commit_transaction` error handling for pymysql (line 240)

#### TC-DB-MYSQL-ERROR-005: MySQLAdapter rollback_transaction с RuntimeError для pymysql
- **Purpose**: Verify rollback_transaction raises RuntimeError when connection is None (pymysql)
- **Preconditions**: MySQLAdapter with pymysql, connection is None
- **Test Steps**:
  1. Create MySQLAdapter with pymysql
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `rollback_transaction` error handling for pymysql (line 249)

#### TC-DB-MYSQL-FALLBACK-001: MySQLAdapter полный fallback сценарий
- **Purpose**: Verify MySQLAdapter fallback from aiomysql to pymysql
- **Preconditions**: aiomysql not available, pymysql available
- **Test Steps**:
  1. Mock aiomysql import to raise ImportError
  2. Create MySQLAdapter
  3. Verify _adapter_type is "pymysql"
  4. Connect to database
  5. Verify pymysql.connect() called
- **Expected Result**: Fallback to pymysql works correctly
- **Coverage**: `_detect_adapter` fallback logic (lines 53, 58)

#### TC-DB-PG-ERROR-001: PostgreSQLAdapter _execute_query_psycopg с RuntimeError
- **Purpose**: Verify _execute_query_psycopg raises RuntimeError when connection is None
- **Preconditions**: PostgreSQLAdapter with psycopg, connection is None
- **Test Steps**:
  1. Create PostgreSQLAdapter with psycopg
  2. Set _connection = None
  3. Call await adapter._execute_query_psycopg("SELECT 1", None)
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `_execute_query_psycopg` error handling (line 162)

#### TC-DB-PG-ERROR-002: PostgreSQLAdapter _execute_query_psycopg с cursor.description is None
- **Purpose**: Verify _execute_query_psycopg returns empty list when cursor.description is None
- **Preconditions**: PostgreSQLAdapter with psycopg, connected
- **Test Steps**:
  1. Create and connect PostgreSQLAdapter with psycopg
  2. Mock cursor.description = None
  3. Call await adapter._execute_query_psycopg("SELECT 1", None)
  4. Verify empty list [] returned
- **Expected Result**: Empty list returned when cursor.description is None
- **Coverage**: `_execute_query_psycopg` edge case (line 166)

#### TC-DB-PG-ERROR-003: PostgreSQLAdapter _execute_command_psycopg с RuntimeError
- **Purpose**: Verify _execute_command_psycopg raises RuntimeError when connection is None
- **Preconditions**: PostgreSQLAdapter with psycopg, connection is None
- **Test Steps**:
  1. Create PostgreSQLAdapter with psycopg
  2. Set _connection = None
  3. Call await adapter._execute_command_psycopg("INSERT INTO users VALUES (1)", None)
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `_execute_command_psycopg` error handling (line 197)

#### TC-DB-PG-ERROR-004: PostgreSQLAdapter begin_transaction с RuntimeError для psycopg
- **Purpose**: Verify begin_transaction raises RuntimeError when connection is None (psycopg)
- **Preconditions**: PostgreSQLAdapter with psycopg, connection is None
- **Test Steps**:
  1. Create PostgreSQLAdapter with psycopg
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `begin_transaction` error handling for psycopg (line 209)

#### TC-DB-PG-ERROR-005: PostgreSQLAdapter commit_transaction с RuntimeError для psycopg
- **Purpose**: Verify commit_transaction raises RuntimeError when connection is None (psycopg)
- **Preconditions**: PostgreSQLAdapter with psycopg, connection is None
- **Test Steps**:
  1. Create PostgreSQLAdapter with psycopg
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `commit_transaction` error handling for psycopg (line 215)

#### TC-DB-PG-ERROR-006: PostgreSQLAdapter rollback_transaction с RuntimeError для psycopg
- **Purpose**: Verify rollback_transaction raises RuntimeError when connection is None (psycopg)
- **Preconditions**: PostgreSQLAdapter with psycopg, connection is None
- **Test Steps**:
  1. Create PostgreSQLAdapter with psycopg
  2. Set _connection = None
  ...
  4. Verify RuntimeError raised: "Connection is not established"
- **Expected Result**: RuntimeError raised
- **Coverage**: `rollback_transaction` error handling for psycopg (line 225)

#### TC-DB-PG-FALLBACK-001: PostgreSQLAdapter полный fallback сценарий
- **Purpose**: Verify PostgreSQLAdapter fallback from asyncpg to psycopg
- **Preconditions**: asyncpg not available, psycopg available
- **Test Steps**:
  1. Mock asyncpg import to raise ImportError
  2. Create PostgreSQLAdapter
  3. Verify _adapter_type is "psycopg"
  4. Connect to database
  5. Verify psycopg.AsyncConnection.connect() called
- **Expected Result**: Fallback to psycopg works correctly
- **Coverage**: `_detect_adapter` fallback logic (lines 54, 59)
