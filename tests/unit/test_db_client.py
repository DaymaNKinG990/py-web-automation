...
from py_web_automation.clients.db_client import DBClient
...
class TestDBClientInit:
    ...
    @allure.title("TC-DB-001: Initialize DBClient with URL, config, and connection_string")
    @allure.description("Test DBClient can be initialized with URL, Config, and connection string. TC-DB-001")
    ...
    @allure.description("Test DBClient can be initialized without connection_string. TC-DB-002")
    ...
    @allure.description("Test DBClient rejects None config (inherited from BaseClient). TC-DB-004")
    def test_init_with_config_none_uses_default(self):
        """Test DBClient uses default config when config=None."""
        with allure.step("Create DBClient with config=None"):
            adapter = SQLiteAdapter("https://example.com/app", None)
            assert adapter.config is not None
            assert adapter.config.timeout == 30  # Default timeout
...
class TestDBClientFactory:
    ...
    def test_create_postgresql_client(self, valid_config, mocker):
        ...
        with allure.step("Mock PostgreSQLAdapter import in db_client"):
            ...
            mock_adapter_class = mocker.patch(
                "py_web_automation.clients.db_adapters.postgresql_adapter.PostgreSQLAdapter"
            )
    ...
    def test_create_mysql_client(self, valid_config, mocker):
        ...
        with allure.step("Mock MySQLAdapter import in db_client"):
            # Mock the import that happens inside DBClient.create()
            mock_adapter_class = mocker.patch("py_web_automation.clients.db_adapters.mysql_adapter.MySQLAdapter")
            mock_instance = MagicMock()
            mock_adapter_class.return_value = mock_instance
    ...
    @allure.description("Test DBClient.create() raises ValueError for unsupported type. TC-DB-008")
    ...
    @allure.description("Test DBClient.create() raises ImportError when library not installed. TC-DB-009")
    ...
    @allure.description("Test DBClient.create() handles case-insensitive db_type. TC-DB-010")
    ...
    @allure.description("Test DBClient.create() handles invalid parameters correctly. TC-DB-045")
    def test_create_with_invalid_parameters(self, valid_config):
        ...
        with allure.step("Call DBClient.create('postgresql', None, config) and verify error"):
    ...
    @allure.description("Test DBClient.create() handles partially invalid parameters. TC-DB-046")
    def test_create_with_partially_invalid_parameters(self, valid_config):
        ...
        with allure.step("Call DBClient.create('postgresql', url, config, connection_string='invalid://')"):
        ...
        with allure.step("Call DBClient.create('sqlite', url, None) - should use default config"):
            # Valid db_type with None config - should create with default config
            db = DBClient.create(
                "sqlite",
                "https://example.com/app",
                None,
            ...
            assert db.config.timeout == 30  # Default timeout
...
class TestDBClientConnection:
    ...
    async def test_close_calls_disconnect(self, valid_config, mocker):
        ...
        with allure.step("Mock disconnect method"):
            mock_disconnect = mocker.patch.object(db, "disconnect", new_callable=AsyncMock)
...
class TestDBClientQueryExecution:
    ...
    @allure.description("Test execute_query() executes SELECT and returns results. TC-DB-017")
    async def test_execute_query_successfully(self, valid_config):
        ...
        with allure.step("Create test table"):
            await db.execute_command("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)")
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Test User"})
    ...
    @allure.description("Test execute_query() returns empty list for no results. TC-DB-018")
    async def test_execute_query_no_results(self, valid_config):
        ...
        with allure.step("Call await db.execute_query(...) with non-existent id"):
            results = await db.execute_query("SELECT * FROM users WHERE id = :id", {"id": 999})
    ...
    @allure.description("Test execute_query() auto-connects if not connected. TC-DB-019")
    ...
    async def test_execute_command_insert(self, valid_config):
        ...
        with allure.step("Call await db.execute_command(...) with INSERT"):
            result = await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Test"})
    ...
    async def test_execute_command_update(self, valid_config):
        ...
        with allure.step("Create test table and insert data"):
            await db.execute_command("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)")
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Original"})
    ...
    async def test_execute_command_delete(self, valid_config):
        ...
        with allure.step("Create test table and insert data"):
            await db.execute_command("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)")
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "To Delete"})

        with allure.step("Call await db.execute_command(...) with DELETE"):
            result = await db.execute_command("DELETE FROM users WHERE id = :id", {"id": 1})
    ...
    @allure.description("Test execute_command() auto-connects if not connected. TC-DB-024")
    async def test_execute_command_auto_connects(self, valid_config, mocker):
        ...
        with allure.step("Call await db.execute_command(...)"):
            # This will fail because we need a real connection, but we can verify connect was called
            try:
                await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Test"})
            except Exception:
                pass  # Expected if connection fails
...
class TestDBClientTransactions:
    ...
    async def test_rollback_transaction_successfully(self, valid_config):
        ...
        with allure.step("Execute INSERT command"):
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "To Rollback"})
    ...
    @allure.description("Test transaction() context manager commits on success. TC-DB-029")
    async def test_transaction_context_manager_success(self, valid_config):
        ...
        with allure.step("Use async with db.transaction():"):
            async with db.transaction():
                await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Test1"})
                await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Test2"})
    ...
    @allure.description("Test transaction() context manager rolls back on error. TC-DB-030")
    ...
    async def test_nested_transactions(self, valid_config):
        ...
        with allure.step("Use nested async with db.transaction():"):
            async with db.transaction():
                await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Outer"})
                async with db.transaction():
                    await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "Inner"})
...
class TestDBClientEdgeCases:
    ...
    @allure.description("Test SQLiteAdapter parses connection string correctly. TC-DB-040")
    def test_sqlite_connection_string_parsing(self, valid_config):
        """Test SQLiteAdapter parses connection string correctly."""
        with allure.step("Create SQLiteAdapter with connection_string='sqlite:///path/to/db.sqlite'"):
            db1 = SQLiteAdapter(
                "https://example.com/app",
                valid_config,
    ...
    async def test_multiple_queries_sequence(self, valid_config):
        ...
        with allure.step("Execute multiple queries in sequence"):
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "User1"})
            await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": "User2"})
            results = await db.execute_query("SELECT * FROM users")
    ...
    @pytest.mark.parametrize(
        "insert_count,expected_count,test_id",
        [
            (0, 0, "TC-DB-LEN-001"),
            (1, 1, "TC-DB-LEN-002"),
            (2, 2, "TC-DB-LEN-003"),
    ...
    @allure.title("{test_id}: DBClient execute_query returns correct result count")
    @allure.description(
        "Test DBClient.execute_query() returns correct number of results for different data sizes. {test_id}"
    )
    async def test_db_client_execute_query_result_count(self, valid_config, insert_count, expected_count, test_id):
        """Test DBClient.execute_query() returns correct number of results for different data sizes."""
        with allure.step(f"Connect to database (insert_count={insert_count})"):
            db = SQLiteAdapter(
                "https://example.com/app",
                valid_config,
                connection_string="sqlite:///:memory:",
        ...
        with allure.step(f"Insert {insert_count} test records"):
            for i in range(insert_count):
                await db.execute_command("INSERT INTO users (name) VALUES (:name)", {"name": f"User{i}"})

        with allure.step("Execute SELECT query"):
            results = await db.execute_query("SELECT * FROM users")

        with allure.step(f"Verify result count is {expected_count}"):
            assert len(results) == expected_count

        with allure.step("Verify all results have correct structure"):
    ...
    @allure.title("TC-DB-043: SQLiteAdapter initialization without connection_string (uses kwargs)")
    @allure.description("Test SQLiteAdapter can be initialized without connection_string using kwargs. TC-DB-043")
    def test_sqlite_adapter_init_without_connection_string_uses_kwargs(self, valid_config):
        """Test SQLiteAdapter can be initialized without connection_string using kwargs."""
        with allure.step("Create SQLiteAdapter without connection_string, with kwargs database_path='test.db'"):
        ...
        with allure.step("Create SQLiteAdapter without connection_string and without database_path in kwargs"):
    ...
    @allure.description("Test SQLiteAdapter raises ImportError when aiosqlite library is not available. TC-DB-044")
    async def test_sqlite_adapter_import_error_when_aiosqlite_missing(self, valid_config, mocker):
        ...
        with allure.step("Verify ImportError message contains helpful installation instructions"):
            assert "aiosqlite" in str(exc_info.value)
            assert "Install it with" in str(exc_info.value) or "uv add aiosqlite" in str(exc_info.value)
...
class TestDBClientAdapters:
    ...
    def test_postgresql_adapter_detects_asyncpg(self, valid_config, mocker):
        ...
        with allure.step("Mock asyncpg import in adapter"):
            mocker.patch(
                "py_web_automation.clients.db_adapters.postgresql_adapter.asyncpg",
                create=True,
            )
    ...
    @allure.description("Test PostgreSQLAdapter uses psycopg when asyncpg not available. TC-DB-033")
        ...
        with allure.step("Mock import to simulate asyncpg missing"):
            ...
            mocker.patch(
                ...
                side_effect=lambda name, *args, **kwargs: (
                    MagicMock() if name != "asyncpg" else (_ for _ in ()).throw(ImportError())
                ),
            )
    ...
    @allure.description("Test MySQLAdapter uses pymysql when aiomysql not available. TC-DB-036")
    ...
    @allure.description("Test SQLiteAdapter begin_transaction() executes BEGIN statement. TC-DB-115")
    ...
    @allure.description("Test DBClient.create() handles errors when config is missing api_id or api_hash. TC-DB-116")
    async def test_db_client_create_with_invalid_config(self, valid_config, mocker):
        """Test DBClient.create() handles errors when config has invalid values."""
        with allure.step("Create Config with invalid timeout"):
            # Config validation will raise ValueError when timeout is invalid
            with pytest.raises(ValueError, match="timeout must be between"):
                Config(
                    base_url="https://example.com",
                    timeout=0,  # Invalid: must be between 1 and 300
                )

        with allure.step("Create Config with invalid retry_count"):
            # Config validation will raise ValueError when retry_count is invalid
            with pytest.raises(ValueError, match="retry_count must be between"):
                Config(
                    base_url="https://example.com",
                    retry_count=11,  # Invalid: must be between 0 and 10
                )

        with allure.step("Call DBClient.create() with config=None"):
            # Test that DBClient.create() properly handles config=None
            # This should use default config (no error)
            db = DBClient.create(
                "sqlite",
                "https://example.com/app",
                config=None,
