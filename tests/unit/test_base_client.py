...
class TestBaseClientInit:
    ...
    @allure.description("TC-BASE-001: Test successful initialization with valid url and config.")
    ...
    @allure.title("TC-BASE-010: BaseClient with config=None uses default")
    @allure.description("TC-BASE-010: Test that BaseClient with config=None uses default Config.")
    def test_init_with_config_none_uses_default(self):
        """Test that BaseClient with config=None uses default Config."""
        with allure.step("Create BaseClient with config=None"):
            app = BaseClient("https://example.com/app", None)
    ...
    @allure.description("TC-BASE-013: Test initialization with empty string URL raises ValueError.")
    def test_init_with_empty_string_url(self, valid_config):
        """Test initialization with empty string URL raises ValueError."""
        with allure.step("Attempt to create BaseClient with empty string URL"):
            with pytest.raises(ValueError, match="url cannot be empty"):
                BaseClient("", valid_config)
    ...
    @allure.description("TC-BASE-011: Test initialization with invalid URL type raises TypeError.")
    def test_init_with_invalid_url_type(self, invalid_url, valid_config):
        ...
        with allure.step(f"Attempt to create BaseClient with invalid URL type: {type(invalid_url).__name__}"):
...
class TestBaseClientContextManager:
    ...
    async def test_aenter_returns_self(self, valid_config):
        ...
        with allure.step("Use async context manager and verify __aenter__ returns self"):
    ...
    @pytest.mark.asyncio
    @allure.title("TC-BASE-007: __aexit__() does not suppress exceptions (default behavior)")
    @allure.description("TC-BASE-007: Test __aexit__() does not suppress exceptions (default behavior).")
    async def test_aexit_does_not_suppress_exceptions(self, valid_config):
        ...
        with allure.step("Create an exception inside the context and verify it is raised"):
...
class TestBaseClientClose:
    ...
    @allure.description("TC-BASE-008: Test close() does not raise exceptions by default.")
    ...
    async def test_close_can_be_overridden_in_subclass(self, mocker, valid_config):
        ...
        with allure.step("Create CustomClient subclass with overridden close"):
        ...
        with allure.step("Create CustomClient instance and mock logger.debug"):
            app = CustomClient("https://example.com/app", valid_config)
...
class TestBaseClientErrorHandling:
    ...
    @allure.description("TC-BASE-016: Test that initialization with non-string URL raises TypeError.")
    ...
    @allure.description("TC-BASE-017: Test that initialization with list as URL raises TypeError.")
    ...
    @allure.title("TC-BASE-018: Config with invalid timeout raises error")
    @allure.description("TC-BASE-018: Test that Config with invalid timeout raises error.")
    def test_init_with_invalid_config_object_raises_at_config_creation(self):
        """Test that Config with invalid timeout raises error."""
        with allure.step("Attempt to create Config with invalid timeout"):
            # Try to create Config with invalid timeout (negative)
            with pytest.raises((TypeError, ValueError)):
                invalid_config = Config(timeout=-1)  # Invalid: timeout must be positive
...
class TestBaseClientLogging:
    ...
    def test_logger_in_subclass_uses_subclass_name(self, valid_config):
        ...
        with allure.step("Create CustomClient subclass"):

            class CustomClient(BaseClient):
                pass

        with allure.step("Create CustomClient instance"):
            app = CustomClient("https://example.com/app", valid_config)

        with allure.step("Verify logger is initialized"):
            # Logger should be bound to CustomClient, not BaseClient
            assert app.logger is not None
            # The logger name should be "CustomClient" (we can't easily verify this without inspecting internals)
...
class TestBaseClientAdditional:
    ...
    @allure.description("TC-BASE-027: Test url attribute is accessible after initialization.")
    ...
    @allure.description("TC-BASE-022: Test config attribute is accessible after initialization.")
    ...
    @allure.description("TC-BASE-023: Test logger attribute is accessible after initialization.")
    ...
    @allure.description("TC-BASE-024: Test full lifecycle: init -> context manager -> close.")
