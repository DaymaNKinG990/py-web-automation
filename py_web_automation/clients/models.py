...
class ApiResult(msgspec.Struct, frozen=True):
    ...
    def assert_status_code(self, expected_code: int) -> None:
        ...
        assert self.status_code == expected_code, (
            f"Expected status code {expected_code}, got {self.status_code}. Response: {self.text()}"
        )

    def assert_success(self) -> None:
        """Assert that request was successful (2xx status)."""
        assert self.success, f"Request failed with status {self.status_code}. Response: {self.text()}"
    ...
    def assert_has_fields(self, *fields: str) -> None:
        ...
        if missing:
            raise AssertionError(f"Missing required fields: {', '.join(missing)}. Response: {data}")
