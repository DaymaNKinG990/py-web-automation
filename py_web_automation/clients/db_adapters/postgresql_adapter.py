...
class PostgreSQLAdapter(DBClient):
    ...
    async def _connect_psycopg(self) -> None:
        ...
        if self.connection_string:
            self._connection = await psycopg.AsyncConnection.connect(self.connection_string)
        else:
            # Build connection from kwargs
            host = self._db_kwargs.get("host", "localhost")
            port = self._db_kwargs.get("port", 5432)
    ...
    async def _execute_query_asyncpg(self, query: str, params: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    async def _execute_query_psycopg(self, query: str, params: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    async def _execute_command_asyncpg(self, command: str, params: dict[str, Any] | None) -> int:
    ...
    async def _execute_command_psycopg(self, command: str, params: dict[str, Any] | None) -> int:
