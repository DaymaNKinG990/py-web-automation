"""
Rate limiting system for GraphQL operations.

This module provides rate limiting functionality using sliding window algorithm
to prevent exceeding GraphQL API rate limits.
"""
...
class RateLimitConfig:
    """
    Configuration for rate limiting GraphQL operations.
...
class RateLimiter:
    """
    Rate limiter using sliding window algorithm for GraphQL operations.

    Tracks GraphQL operations in a sliding time window and blocks operations
    ...
    async def acquire(self) -> None:
        """
        Acquire permission to make a GraphQL operation.
    ...
    async def try_acquire(self) -> bool:
        ...
        async with self._lock:
            ...
            while self.requests and self.requests[0] < window_start:
                self.requests.popleft()
            # Check if we can make an operation
    ...
    def reset(self) -> None:
        """Reset rate limiter (clear all operation history)."""
        self.requests.clear()
