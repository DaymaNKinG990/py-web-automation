"""
Middleware system for SOAP request/response interception and modification.

This module provides a middleware/interceptor system that allows hooking into
the SOAP operation lifecycle for logging, metrics, validation, and modification.
"""
...
class Middleware(ABC):
    """
    Base class for SOAP middleware implementations.

    Middleware can intercept and modify SOAP operations before they are sent
    and responses after they are received.

    Example:
        >>> class LoggingMiddleware(Middleware):
        ...     async def process_request(self, context: _SoapRequestContext) -> None:
        ...         print(f"Request: {context.operation}")
    ...
    async def process_request(self, context: _SoapRequestContext) -> None:
        ...
        Can modify context.headers, context.body, etc.
    ...
    async def process_error(
        ...
        Can return a modified SoapResult to replace the error response,
        ...
        Returns:
            Optional SoapResult to replace error, or None to propagate error
        """
        return None
...
class MiddlewareChain:
    """
    Chain of middleware to process SOAP operations.
    ...
    async def process_error(
        ...
        Returns:
            Optional SoapResult to replace error, or None
        """
        for middleware in reversed(self._middleware):
